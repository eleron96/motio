...
Then:
- текстовый контент задачи сдвигается внутрь видимой части плашки только если сама задача действительно длинная по timeline и заголовок полностью ушел за левую границу viewport;
- после того как видимая часть заголовка дошла до левой границы доступной области, дальнейший сдвиг внутри блока не продолжается;
- текстовый контейнер ограничен шириной реально видимого сегмента, поэтому подпись не уезжает глубже внутрь плашки после достижения левой границы;
- если заголовок уже виден хотя бы частично, базовое положение подписи и project line не меняются;
- короткие частично обрезанные задачи не меняют базовое положение подписи;
- если shifted-задача остается видимой, но свободной ширины уже мало, заголовок переводится в компактный двухстрочный режим до безопасного лимита вместо раннего `truncate`;
- в этом компактном режиме project line скрывается, чтобы максимум места оставался у заголовка;
- только в shifted-состоянии при узкой видимой части скрывается project line, чтобы приоритет оставался у title;
