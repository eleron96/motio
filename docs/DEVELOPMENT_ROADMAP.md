...
### 1.1 Онбординг (Welcome Flow)

**Зачем:** Сейчас после логина — пустой таймлайн. 15-30 минут до Aha-момента. Цель: 2-3 минуты.

```
Задачи:
[ ] Welcome Screen после первого входа в новый workspace:
    "Добро пожаловать в Motio! Твой воркспейс создан."
    Кнопки: [Создать задачу] [Пригласить команду] [Посмотреть тур]

[ ] Onboarding Tour (3 шага, библиотека react-joyride или driver.js):
    Шаг 1: "Здесь твой таймлайн — перетягивай задачи" → highlight timeline grid
    Шаг 2: "Фильтруй по проектам и людям" → highlight filter panel
    Шаг 3: "Пригласи команду" → highlight members link

[ ] Флаг has_completed_onboarding в user preferences
