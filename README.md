...
- Logout идёт по цепочке `/oauth2/sign_out` -> `Keycloak end-session` -> `/auth?silent=1`, чтобы не оставлять живую SSO-сессию.
...
- `OAUTH2_PROXY_WHITELIST_DOMAINS` — allowlist доменов для `rd` redirect после `/oauth2/sign_out` (нужен для logout-chain через Keycloak).
