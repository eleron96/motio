...
## Scenario 7: Project search is identical across task and milestone forms

Given:
- пользователь открывает создание задачи, редактирование существующей задачи и создание/edit milestone;
- во всех формах доступен выпадающий список проектов.

When:
- пользователь вводит символы в строку поиска проектов внутри списка.

Then:
- во всех формах применяется одинаковая фильтрация по `name`/`code`;
...
- `src/features/planner/components/timeline/MilestoneDialog.tsx`
